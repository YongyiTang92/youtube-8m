...
class cc_willow_MoeModel(models.BaseModel):
  """A softmax over a mixture of logistic models (with L2 regularization)."""
  ...
  def create_model(self,
    ...
    experts_gating_weights = tf.get_variable("expert_gating_prob_weights",
                                         [vocab_size, vocab_size*num_mixtures],
                                         initializer=tf.random_normal_initializer(stddev=1 / math.sqrt(vocab_size)))
    experts_gates = tf.matmul(model_input, experts_gating_weights)
    experts_gates = slim.batch_norm(
          experts_gates,
          center=True,
          scale=True,
          is_training=is_training,
          scope="expert_gating_prob_bn")

    gating_distribution = tf.nn.softmax(tf.reshape(
        gate_activations,
        [-1, num_mixtures + 1]))  # (Batch * #Labels) x (num_mixtures + 1)
    expert_distribution = tf.nn.sigmoid(tf.reshape(
        expert_activations*experts_gates,
        [-1, num_mixtures]))  # (Batch * #Labels) x num_mixtures
...
class res_willow_MoeModel(models.BaseModel):
  """A softmax over a mixture of logistic models (with L2 regularization)."""
  ...
  def create_model(self,
    ...
    expert_activations_hidden = slim.batch_norm(
        expert_activations_hidden,
        center=True,
        scale=True,
        is_training=is_training,
        scope="experts_hidden_bn")
    expert_activations_hidden = slim.dropout(expert_activations_hidden, 0.5, scope='dropout')
